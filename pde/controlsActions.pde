...
void button_mode_drawTesla()
{

  println("Draw Tesla Button");
  resetQueue();
  sendSetHomePosition();
  addToCommandQueue(CMD_PENUP + penLiftUpPosition +",END");
    
  // Select image
  // TODO: Dynamic path somehow.
 
  RShape shape = loadShapeFromFile("Em_Tesla_INCH.svg");

  if (shape != null) 
  {
    setVectorFilename("Em_Tesla_INCH.svg");
    setVectorShape(shape);
  }
  else 
  {
     println("File not found (Em_Tesla_INCH.svg)");
  }
 
  button_mode_renderVectors();
  addToCommandQueue(CMD_PENUP + penLiftUpPosition +",END");
  button_mode_returnToHome();
  button_mode_pauseQueue();
    // Draw vectors
    // Pen lift
    // Return to home
    // Unpause
